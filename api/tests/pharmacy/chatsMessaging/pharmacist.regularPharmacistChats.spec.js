...
const testOrderId = 88;
const testThreadId = 80;

function sendMessageDetails() {
  const sender = `PHARMACIST`;
  const message = `Sample message from api - ${randomAlphanumeric(3)}`;
}
...
test.describe('GraphQL: Regular Pharmacist Messaging', () => {
  ...
  test(
    'PHARMA-197 | Should be able to GET Chat details as regular pharmacist for NON-onboarded orders',
    {
  ...
  test(
    'PHARMA-202 | Should be able to send Chat message by Order ID as Regular Pharmacist',
    {
