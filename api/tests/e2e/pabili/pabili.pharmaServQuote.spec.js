...
test.describe('GraphQL E2E Workflow: Pabili Happy Path (Pharmaserv Sends Quote)', () => {
  ...
  test(
    'PHARMA-346 | Pabili happy path with pharmaserv quote and quantity reduction at pay order',
    {
      tag: ['@api', '@workflow', '@pabili', '@patient', '@pharmacist', '@rider', '@admin', '@positive', '@pharma-346'],
    },
    async ({ api }) => {
      const paymentQrImagePath = path.resolve('upload/images/qr1.png');
      const patientProofPaymentImagePath = path.resolve('upload/images/proof1.png');
      ...
      // Patient: Accept Quote and collect quoted prescription item IDs.
      ...
      // Patient: Pay Order with reduced quantities (set quantity=4 for all quoted medicine items).
      await payOrderAsPatient(api, {
        patientAccessToken,
        orderId,
        proof: {
