...
test.describe('GraphQL: Patient Send Chat Message By Thread Id', () => {
  ...
  test(
    'PHARMA-489 | Send chat message by thread id should satisfy response contract shape',
    {
      tag: ['@api', '@patient', '@positive', '@pharma-489'],
    },
    async ({ api }) => {
      const { accessToken, raw: loginRes } = await loginAsPatientAndGetTokens(api, getPatientCredentials('default'));
      expect(loginRes.ok, loginRes.error || 'Patient login failed').toBe(true);
      ...
      const node = sendChatMessageRes.body?.data?.patient?.chat?.sendThreadMessage;
      expect(node, 'Missing data.patient.chat.sendThreadMessage').toBeTruthy();
